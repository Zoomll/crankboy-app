...
#define INFO "Press Ⓐ to toggle patches.\n \nHold Ⓐ to rearrange patches. Enabled patches will be applied in the order listed."
...
static void PGB_PatchesScene_update(void* object, uint32_t u32enc_dt)
{
    ...
    // header
    {
        ...
        playdate->graphics->drawText(
            name, strlen(name), kUTF8Encoding, textX, textY
        );
    }
    ...
    // menu movement
    int ydir = !!(PGB_App->buttons_pressed & kButtonDown) - !!(PGB_App->buttons_pressed & kButtonUp);
    if ((ydir < 0 && patchesScene->selected > 0) || (ydir > 0 && patchesScene->selected < len-1))
    {
        if (held)
        {
            // reorder patches
            memswap(&patchesScene->patches[patchesScene->selected], &patchesScene->patches[patchesScene->selected + ydir], sizeof(SoftPatch));
    ...
    for (size_t i = 0; scroll + i < len && i < MAX_DISP; ++i)
    {
        ...
        playdate->graphics->drawRect(MARGIN, y + ROW_HEIGHT/2 - BOX_SIZE/2, BOX_SIZE, BOX_SIZE, kColorBlack);
        playdate->graphics->drawRect(MARGIN+1, y + ROW_HEIGHT/2 - BOX_SIZE/2+1, BOX_SIZE-2, BOX_SIZE-2, kColorBlack);
        
        if (patch->state == PATCH_ENABLED || thisHeld)
        {
            LCDColor col = thisHeld ? (uintptr_t)&lcdp_50 : kColorBlack;
            playdate->graphics->fillRect(
                MARGIN+BOX_SELECTED_PADDING, y + ROW_HEIGHT/2 - BOX_SIZE/2+BOX_SELECTED_PADDING, BOX_SIZE-2*BOX_SELECTED_PADDING, BOX_SIZE-2*BOX_SELECTED_PADDING, col
            );
        }
        
        // TODO: display "new" patches slightly differently
        
        playdate->graphics->drawText(patch->basename, strlen(patch->basename), kUTF8Encoding, MARGIN*2 + BOX_SIZE, y + ROW_HEIGHT_TEXT_OFFSET);
    ...
    playdate->graphics->drawTextInRect(
        info, strlen(info), kUTF8Encoding, MARGIN, INFO_Y, LCD_COLUMNS - 2*MARGIN, 200, kWrapWord, kAlignTextLeft
    );
...
static void PGB_PatchesScene_free(void* object)
{
    ...
    {
        if (patch->state < 0) patch->state = PATCH_DISABLED;
    }
